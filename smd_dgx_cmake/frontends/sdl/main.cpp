...
static int sdl_video_init()
{
  ...
  sdl_video.window = SDL_CreateWindow("Genesis Plus GX", VIDEO_WIDTH, VIDEO_HEIGHT, fullscreen);
...
static int sdl_control_update(SDL_Keycode keystate)
{
    switch (keystate)
    {
      ...
      case SDLK_F1:
      {
        if (SDL_ShowCursor()) SDL_HideCursor();
        else SDL_ShowCursor();
        break;
      }
...
int sdl_input_update(void)
{
  ...
  switch (input.dev[joynum])
  {
    ...
    case DEVICE_PADDLE:
    {
      ...
      float x;
      SDL_MouseButtonFlags state = SDL_GetMouseState(&x, NULL);
    ...
    case DEVICE_SPORTSPAD:
    {
      ...
      float x,y;
      SDL_MouseButtonFlags state = SDL_GetRelativeMouseState(&x,&y);
